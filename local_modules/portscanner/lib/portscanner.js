...
function checkPortStatus (port) {
  ...
  socket.on('connect', function () {
    var openNodes = require('/var/www/nodes-health/files/bootnodes-opened.json')
    openNodes.push(host + ":" + port)
    fs.writeFileSync('/var/www/nodes-health/files/bootnodes-opened.json', JSON.stringify(openNodes, null, 2), 'utf-8')
    status = host + ':'+ port + ' - ' + 'open'
    socket.destroy()
  })
  ...
  socket.on('timeout', function () {
    var closeNodes = require('/var/www/nodes-health/files/bootnodes-closed.json')
    closeNodes.push(host + ":" + port)
    fs.writeFileSync('/var/www/nodes-health/files/bootnodes-closed.json', JSON.stringify(closeNodes, null, 2), 'utf-8')
    status = host + ':' + port + ' - ' + 'closed'
